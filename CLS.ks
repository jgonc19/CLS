...
set dataLogging to launchParameters[7].
...
set averageisp to 0.
set missionTimeLog to 0.																	// Tracks time for use in data logging function
...
HUDinit(launchtime,targetapoapsis,targetperiapsis,targetinclination, dataLogging).				// Prints info at top of HUD
...
until runmode > 0 {
	...
	If cdown >= 0 and tminus = 0 and throttle >= liftoffThrottle*0.985 {
		If vesTWR < minLiftoffTWR {
			...
			lock missionElapsedTime to round(time:seconds - launchTime,2).
			if vehicleConfig = 1 {	
	...
	if runmode = -1 {
		...
		if proceedMode = 1 {													// continue countdown
			...
			HUDinit(launchtime,targetapoapsis,targetperiapsis,targetinclination,dataLogging).
...
when ship:verticalspeed < -1.0 and ship:altitude < atmAlt then {
	if runmode = 3 { return false. }
...
if dataLogging { 
	logInitialise(targetapoapsis,targetperiapsis,targetinclination).
}
set warpLimit to 1.
...
until ship:verticalspeed > 100 {
	...
	StageDV(). 
	if dataLogging and missionElapsedTime > missionTimeLog {log_data().}
	wait 0.01.
}
set runmode to 2.
...
if dataLogging { 
	Print "------------" at (3,39).
	scrollprint("Data Log Complete",false).
}
set runmode to 3.
...
Function stagingDetection {
	...
	local stageTime is time:seconds.
	...
	if stageflameout {
		...
		until time:seconds > stageTime + stagedelay {
			if dataLogging and missionElapsedTime > missionTimeLog {log_data().}
			wait 0.01.
		}
		StageJettison().
		...
		until time:seconds > stageTime + jettisondelay {
			if dataLogging and missionElapsedTime > missionTimeLog {log_data().}
			wait 0.01.
		}
		BoosterJettison().
...
Function FairingJettison {
	If (Stage:number - PayloadProtectionStage)=1 and currentstagenum < maxStages {
